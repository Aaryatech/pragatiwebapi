...
public interface GetFeedHeaderRepo extends JpaRepository<GetFeedHeader, Integer> {
	...
	@Query(value = "SELECT d.*,s.sa_name,u.user_name FROM t_fb_header d ,m_sa s,m_user u WHERE d.del_status=0 AND d.fb_date BETWEEN :fromDate AND :toDate AND d.status=:status AND d.company_id=:companyId AND d.sa_id=:saId  AND d.sa_id=s.sa_id AND d.rel_man_id=u.user_id", nativeQuery = true)
	...
	@Query(value = "SELECT d.*,s.sa_name,u.user_name FROM t_fb_header d,m_sa s,m_user u WHERE d.del_status=0 AND d.fb_date BETWEEN :fromDate AND :toDate  AND d.company_id=:companyId AND d.sa_id=:saId  AND d.sa_id=s.sa_id AND d.rel_man_id=u.user_id", nativeQuery = true)
	...
	@Query(value = "SELECT d.*,s.sa_name,u.user_name FROM t_fb_header d,m_sa s,m_user u WHERE d.del_status=0 AND d.fb_date BETWEEN :fromDate AND :toDate  AND d.company_id=:companyId AND d.sa_id=s.sa_id AND d.rel_man_id=u.user_id", nativeQuery = true)
	List<GetFeedHeader> getFeedbackListAllStatusAndSaId(@Param("companyId") int companyId,
			@Param("fromDate") String fromDate, @Param("toDate") String toDate);

	@Query(value = "SELECT d.*,s.sa_name,u.user_name FROM t_fb_header d ,m_sa s,m_user u WHERE d.del_status=0 AND d.fb_date BETWEEN :fromDate AND :toDate AND d.status=:status AND d.company_id=:companyId AND d.sa_id=s.sa_id AND d.rel_man_id=u.user_id", nativeQuery = true)
